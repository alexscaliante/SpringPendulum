...
function MouseCamController(camera, canvas, camTarget) {
   ...
   // and is registered with the mouse wheel. The magic numbers have been
   // found by trial and error to get smooth behaviour.
   function mousewheelCB (event) {

      event.preventDefault();
      // make sure we can't zoom when the mouse is not over the canvas
   ...
   function mousedown( event ) {
      ...
      // make sure we can't rotate when the mouse is not over the canvas
      if ((event.clientX < canvas.width) && (event.clientY < canvas.height)) {
